...
describe('Application Users Handler', async () => {
  ...
  it('authenticate should be response with a created user token', () => {
    return chai
      .request(app)
      .post(
        '/users/authenticate?firstname=test&lastname=test2&password=test123'
      )
      .set('Accept', 'application/json')
      .then(async (res) => {
        chai.expect(res.status).to.equals(200)
  ...
  it('show should be response with a single user or empty on call', async () => {
    const user = await appUser.authenticate(
